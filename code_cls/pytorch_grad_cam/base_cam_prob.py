...
class BaseCAM:
    ...
    def get_cam_image(self,
        ...
        acti = torch.from_numpy(activations)
        acti_soft = torch.softmax(acti, dim=1)
        acti_soft = np.array(acti_soft)

        return cam, acti_soft
    ...
    def forward(self, input_tensor, target_category=None, eigen_smooth=False):
        ...
        # or something else.
        cam_per_layer, prob_per_layer = self.compute_cam_per_layer(input_tensor,
                                                   target_category,
                                                   eigen_smooth)
        return self.aggregate_multi_layers(cam_per_layer), target_category, prob_per_layer[0]
    ...
    def compute_cam_per_layer(
        ...
        for target_layer, layer_activations, layer_grads in zip(self.target_layers, activations_list, grads_list):
            ...
            scaled_prob = self.scale_cam_image(probs[0], target_size)
            cam_per_target_layer.append(scaled[:, None, :])
            probs_per_target_layer.append(scaled_prob)
        return cam_per_target_layer, probs_per_target_layer
